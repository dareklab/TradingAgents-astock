...
- **短线交易逻辑适配** — 默认持股周期从 5 天改为 3 天（1-3 天，不超过 5 天）。新增
  `holding_days` 配置项。Portfolio Manager 和 Trader 的 System Prompt 重写为短线风格：
  优先游资/资金流/政策催化的短线信号，长期基本面降为次要；止损 -3%~-5%、止盈 +3%~+5%。
  `time_horizon` 字段描述改为 `"1-3 trading days"`。
  —— `default_config.py` / `trading_graph.py` / `portfolio_manager.py` / `trader/trader.py` / `schemas.py`

- **数据源迁移：彻底移除 push2.eastmoney.com 依赖** — 
  - `push2/qt/stock/get`（行业/股本/市值）→ mootdx `finance()` + 腾讯财经市值
  - `push2/qt/clist/get`（行业排名）→ 百度股市通概念板块
  - `push2/qt/stock/fflow/kline/get`（实时资金流）→ mootdx OHLCV 计算 Money Flow 代理指标
  - `push2his/.../daykline/get`（历史资金流）→ 同上
  —— `a_stock.py`（`get_fundamentals` / `get_industry_comparison` / `get_fund_flow`）

### Fixed

- **任务队列/分析窗口股票名称显示为空** — 后端 `api_analyze` 提交任务时立即解析
  `display_name`；前端任务列表轮询中同步 `displayName` 状态。
  —— `backend/main.py` / `frontend/src/App.tsx`

- **`_em_get` 重试日志降噪** — 重试日志从 INFO 降为 DEBUG。
  —— `a_stock.py`

### Changed

- **README.md 重写** — 精简重复内容（472→140 行），新增部署/LLM 供应商/CLI 章节，
  删除无用 assets（10 个文件），项目结构按实际目录重新整理。
